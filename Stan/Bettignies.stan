...
parameters{
  ...
  vector[n_treatment] log_delta_z; // z-scores
...
transformed parameters{
  // Convert z-scores
  vector[n_treatment] log_delta = log_delta_z * log_delta_sigma + log_delta_mu;
...
model{
  ...
  log_delta_mu ~ normal( log(0.01) , 0.2 );
  log_mu_mu ~ normal( log(30) , 0.3 );
  log_tau_mu ~ normal( log(0.02365) , 0.5 );
  log_delta_sigma ~ normal( 0 , 0.2 )T[0,]; // half-normal priors
  log_mu_sigma ~ normal( 0 , 0.3 )T[0,];
  log_tau_sigma ~ normal( 0 , 0.5 )T[0,];
  
  // Priors for parameters describing treatment mean
  log_delta_z ~ normal( 0 , 1 ); // standard normal prior for z-scores
  log_mu_z ~ normal( 0 , 1 );
  ...
  // Model
  // Parameters
  vector[n] delta = exp( log_delta[treatment] );
